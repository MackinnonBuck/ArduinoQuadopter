...
uint8_t MPU6050Manager::dmpInitialize() {
    ...
    if (m_mpu.writeProgMemoryBlock(dmpMemory, MPU6050_DMP_CODE_SIZE)) {
        if (m_mpu.writeProgDMPConfigurationSet(dmpConfig, MPU6050_DMP_CONFIG_SIZE)) {
            ...
            m_mpu.writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
            ...
            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
            m_mpu.readMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
		...
		int16_t xGyroOffset, int16_t yGyroOffset, int16_t zGyroOffset)
{
	...
	Serial.println(m_mpu.testConnection() ? "MPU6050 connection successful" : "MPU6050 connection failed"); // FIX
